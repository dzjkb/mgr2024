...
LOG_EVERY_TRAIN_STEP = 50  # log every n-th step to limit GPU memory syncs and speed up training
            ...
            *encoder_blocks,
            ...
            _weightnorm(
                nn.Conv1d(
                    ...
                    latent_size * 2,  # outputs mean and scale for each dim
...
class _DecoderBlock(nn.Module):
    ...
    def __init__(
        ...
        assert in_channels % 2 == 0, f"_DeocderBlock got an odd number of channels: {in_channels}"
...
class VAE(pl.LightningModule):
    def __init__(
        ...
        self.training_outputs: dict[str, list[Tensor]] = {
            "original": [],
            "reconstruction": [],
        }
    ...
    def _reparametrize(self, mean: Tensor, scale: Tensor) -> Tensor:
        ...
        # logvar = torch.clamp(predicted_logvar, min=-5, max=10)  # enforce minimal variance on each latent dimension
    ...
    def _latent_loss(self, mean: Tensor, scale: Tensor, batch_idx: int | None = None) -> Tensor:
        std = self._std(scale)
        ...
        if batch_idx is not None and batch_idx % LOG_EVERY_TRAIN_STEP == 0:
    ...
    def forward(
        self, x: Tensor, discriminator_step: bool = False, batch_idx: int | None = None,
        ...
        if not discriminator_step:
            losses_dict = {
                "latent_loss": (
                    self._latent_loss(mean, scale, batch_idx=batch_idx)
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        self.training_outputs["original"] = []
        self.training_outputs["reconstruction"] = []
...
def _random_z(n: int, latent_size: int, device: torch.device) -> Tensor:
    return torch.distributions.Normal(
        torch.zeros(size=(n, latent_size, 1)),
        torch.ones(size=(n, latent_size, 1)),
    ).sample().to(device)
...
def generate_with_model(
    ...
    with torch.no_grad():
        z = _random_z(count, latent_size, model.device)
